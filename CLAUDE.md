...
The app's colors are centralized in `src/index.css`'s Tailwind v4 `@theme` block, which remaps generic Tailwind color names to this app's actual palette - e.g. `orange-500`/`600`/`700` and the custom `--color-brand`/`--color-brand-hover`/`--color-gold` tokens are the **primary brand accent** (buttons, active/selected states, links, icons), a muted, deliberately desaturated indigo→teal gradient (`#8B8FF7` → `#7DD3D8`, see `--gradient-brand`) - kept intentionally soft rather than a saturated/neon color, since a dark background plus a vivid accent reads as "bright" regardless of hue (this is the second iteration of the palette; the first pass used a more saturated `#6366F1`/`#22D3EE` and read as "the same as before" against the dark background, hence the desaturation). `gray-50`/`100` (`#0a0a12`/`#14141f`) are the near-black page/surface background. Most of the app (all four dashboards, Checkout's detail rows, Orders/Profile/Tracking pages) reads these tokens via plain Tailwind classes (`bg-orange-500`, `text-brand`, etc.), so a future palette change is mostly a one-file edit in `index.css` - but **`HomePage.tsx`, `CategoryPage.tsx`, `FoodInfoPage.tsx`, `AuthPage.tsx`, `CityGateway.tsx`, and a few others hardcode the brand hex directly** instead of the semantic classes - those needed individual find/replace and won't pick up a future token-only change. A second, unrelated hardcoded orange (`#FC8019`, Swiggy-orange) that a few files used independently of the theme was also folded into the brand token.
...
- **2026-09-22 (7)** — Follow-up on (6) based on direct feedback after seeing it running: the color pass "looked the same as before," the visible payment options were missing, and the app had emoji scattered through toasts/badges/headings that read as unpolished for a SaaS-styled product.
  - **Palette desaturated**: the brand accent moved from a saturated `#6366F1`/`#22D3EE` to a muted `#8B8FF7`/`#7DD3D8` - lower saturation, lower glow opacity (`--color-brand-glow`/`--color-accent-glow` cut roughly in half). A vivid accent on a near-black background reads as "bright" regardless of hue, which is why the first pass didn't land - see "Design system / color theme" above.
  - **`Checkout.tsx`'s payment method panel was never touched in (6)**: it used its own independent `emerald-500`/`teal-500` literal Tailwind colors (not the `orange-500`/`brand` token), so the theme-token change in (6) had zero effect on the single most-seen part of the checkout flow - the selected-payment-method ring/glow, the "Select Payment Method" header, and the final "Place Order" CTA button gradient all still looked like the old theme. All converted to the brand token; the four payment cards (GPay/PhonePe/Cards/Cash on Delivery) now animate in with a `framer-motion` stagger and respond to hover/tap, and the CTA button uses `whileTap` instead of a CSS `active:` transform. Cash on Delivery is the only option visible without `VITE_RAZORPAY_KEY_ID` set (by design, see Payments section) - the GPay/PhonePe/Cards cards were redesigned regardless so they're ready once a real key is configured.
  - **Removed decorative emoji app-wide** (toasts, badges, headings, greeting banners) - kept only glyphs that are actual UI (the ✕/✓ used as button/step-indicator content, which aren't emoji and were never touched) and the Telegram order-notification message builder in `Checkout.tsx` (ops-facing text sent to a Telegram bot, not part of the app's own UI, deliberately left alone). A few spots where an emoji was the *entire* visual content of an element (a status dot, a map-pin badge, an empty-state icon, the King/Queen persona picker's crown) got a real `lucide-react` icon in its place instead of just going blank.
  - Verified with `tsc --noEmit` and a full `vite build`.
  - **Process note for next time**: the first attempt at the emoji removal used a single find/replace pass that also collapsed whitespace next to *every* quote/bracket in each file, not just ones adjacent to a removed emoji - this corrupted formatting across all 27 files it touched (still valid JS, since most of that whitespace isn't syntactically significant, but not something to leave in the codebase). Caught before committing by diffing against HEAD; fixed by reverting via `git checkout` and re-running a corrected version that only touches whitespace immediately adjacent to an actual removed emoji. If doing a similar bulk text transform again, test the regex against a handful of representative lines (including ones with no emoji at all) before running it across real files.
